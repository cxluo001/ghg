...
# Convert DataFrame to CSV (fixes NameError)
...
def extract_boq_from_pdf(pdf_path):
    ...
    # If STILL no results, try OCR
    if not extracted_data_frames:
        text = extract_text_with_ocr(pdf_path)
        ocr_df = extract_material_rows_from_text(text)
        
        if not ocr_df.empty:
            extracted_data_frames.append(ocr_df)
...
def extract_boQ_from_pdfs(pdf_file, section_name="Bill of Quantities"):
    """
    Main function to extract BOQ data from uploaded PDFs.
    ...
    extracted_df = extract_boq_from_pdf(temp_pdf_path)
    ...
    country_name = extract_country_from_text(all_text, country_to_region)
    ...
    # Add the source PDF name (use the original file name)
    extracted_df["Source PDF"] = pdf_file.name  # Use the original file name
...
def main():
    ...
    st.sidebar.header("Upload Bill of Quantities (BoQ) PDFs")
    boq_files = st.sidebar.file_uploader("Upload BoQ PDF files", type=["pdf"], accept_multiple_files=True)
    ...
    if emission_file:
        ...
        if boq_files:
            ...
            for boq_file in boq_files:
                ...
                # Pass the original file object (boq_file) directly
                extracted_df, country_name = extract_boQ_from_pdfs(boq_file)
                ...
                if extracted_df.empty:
                    st.warning(f"No concrete, steel, or asphalt materials detected in {boq_file.name}.")
                    continue
                
                # Ensure required columns exist
                required_columns = ["Material", "Quantity", "Unit"]
                missing_columns = [col for col in required_columns if col not in extracted_df.columns]
                
                if missing_columns:
                    st.warning(f"Missing required columns in {boq_file.name}: {', '.join(missing_columns)}")
                    continue
                
                all_boq_data.append(extracted_df)
            
            if all_boq_data:
                # Combine all extracted BOQ data
                combined_boq_df = pd.concat(all_boq_data, ignore_index=True)
                
                # Display summary of extracted materials
                st.success(f"Successfully extracted {len(combined_boq_df)} material items focusing on concrete, steel, and asphalt.")
                
                # Get the region based on detected countries
                country_name = next(iter(all_countries), None) if all_countries else None
                region = country_to_region.get(country_name, "Global")
                
                # Process emission factors focusing on target materials
                ...
                if processed_df is not None:
                    ...
                    # Match BOQ materials with emission categories
                    ...
                    if emissions_data:
                        ...
                        # Set up index for display
                        emissions_df.index = emissions_df.index + 1
                        
                        # Display dashboard with emissions data and detected countries
                        display_dashboard(emissions_df, all_countries)
                        
                        # Allow manual category editing
                        updated_emissions_df = edit_categories_interactive(emissions_df, processed_df)
                        
                        # Display updated emissions table
                        st.markdown(
                            f"""
                            <h1 style='color: rgb(0, 37, 105); font-size: 28px;'>Updated Emissions Table</h1>
                            """,
                            unsafe_allow_html=True
                        )
                        
                        # Style the updated table
                        ...
                        st.error("No emissions data could be calculated. Please check if the BOQ items match available emission factor categories.")
                else:
                    st.error("Could not process emission factors. Please check the format of your emission factors file.")
            else:
                st.error("No valid BOQ data could be extracted from the uploaded PDFs.")
        else:
            st.info("Please upload one or more PDF files containing Bills of Quantities.")
    else:
        st.info("Please upload an emission factors Excel file to begin.")
